...
# -*- coding: utf-8 -*-
"""
Version simplifiée du scraper sans MongoDB - pour test
"""
...
def fetch_articles(url):
    ...
    try:
        ...
        articles = main_tag.find_all('article')[:30]  # Extraire 30 articles
...
if __name__ == "__main__":
    ...
    print("Récupération des articles du Blog du Modérateur...")
    articles = fetch_articles(url)

    # Affichage des articles récupérés
    for i, article in enumerate(articles, 1):
        print(f"\n{'='*60}")
        print(f"ARTICLE {i}: {article.get('title', 'Sans titre')}")
        print(f"{'='*60}")
        print(f"Catégorie: {article.get('subcategory', 'N/A')}")
        print(f"Auteur: {article.get('author', 'N/A')}")
        print(f"Date: {article.get('date', 'N/A')}")
        print(f"URL: {article.get('url', 'N/A')}")
        print(f"Images: {len(article.get('images', {}))} trouvées")
        
        if article.get('summary'):
            print(f"Résumé: {article['summary'][:150]}...")
        
        if article.get('content'):
            print(f"Contenu: {article['content'][:300]}...")

    # Sauvegarde en JSON
    save_to_json(articles)
    
    print(f"\n{len(articles)} articles traités avec succès!")
